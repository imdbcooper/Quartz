...
> **Краткая справка:** Подключение внешнего провайдера эмбеддингов (например, через прокси типа Polza, OpenRouter или локальный сервер) позволяет использовать более мощные или дешевые модели для семантического поиска по вашей базе заметок.
