...
As you already have Quartz locally, you don't need to fork or clone it again. In this fork, stay on your working branch (usually `main`), install the dependencies, and import your old vault.
...
If you intentionally track the original Quartz project and get an error like `fatal: 'upstream' does not appear to be a git repository`, add it as a separate remote:

```shell
git remote add upstream https://github.com/jackyzha0/quartz.git
```

After that, run `npx quartz create` and choose how to initialize your content folder. You can import or link your previous content folder and Quartz should work much like before.
...
3. **Replacing Go template syntax with JSX**: Quartz 3 used [Go templates](https://pkg.go.dev/text/template) to create layouts for pages. However, the syntax isn't great for doing more complex rendering and it got very difficult to make meaningful layout changes to Quartz 3. Quartz 4 uses an extension of JavaScript syntax called JSX which allows you to write layout code that looks like HTML in JavaScript which is significantly easier to understand and maintain.
...
- Ensure that your default branch on GitHub is updated from `hugo` to your active Quartz 4 branch (for this fork, `main`).
